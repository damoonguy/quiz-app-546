    ...
    app.route('/register')
        ...
        .post(async (req, res) => {
            try {
                ...
                res.redirect(newUser.role === 'admin' ? '/admin' : '/users');
            } catch (e) {
    ...
    app.route('/quiz/create')
        ...
        .post(async (req, res) => {
            try {
                ...
                const creatorCollection = await users();
                ...
                const updatedUser = await creatorCollection.findOneAndUpdate({_id: new ObjectId(req.session.user._id)}, {$push: {createdQuizzes: insertInfo.insertedId}}, {ReturnDocument: 'after'})

                res.redirect(req.session.user.role === 'admin' ? '/admin' : '/users');
            } catch (e) {
